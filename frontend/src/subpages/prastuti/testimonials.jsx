...
const Testimonials = () => {
  ...
  const renderTestimonialCard = (testimonial, index) => (
    <div
      ...
      className="bg-white rounded-2xl p-4 md:p-6 lg:p-6 transition-shadow duration-300 flex-shrink-0 mx-auto min-h-[400px] md:min-h-[420px] lg:min-h-[400px] flex flex-col"
    >
      <div className="relative mb-4 lg:mb-5 flex-shrink-0">
        {/* Blue gradient semi-circle */}
        <div
          className="w-20 h-10 md:w-24 md:h-12 lg:w-20 lg:h-10 mx-auto rounded-t-full"
          style={{
      ...
      <div className="text-center pt-3 lg:pt-4 flex-1 flex flex-col">
        <h3 className="text-blue-800 text-base md:text-lg lg:text-base font-bold mb-1 flex-shrink-0">
          {testimonial.name}
        </h3>
        <p className="text-gray-500 text-xs md:text-sm lg:text-xs mb-3 md:mb-4 lg:mb-3 flex-shrink-0">
          {testimonial.title}
        </p>
        ...
        <div className="flex justify-center space-x-1 flex-shrink-0">
          {[...Array(testimonial.rating)].map((_, i) => (
            <svg
              key={i}
              className="w-4 h-4 md:w-5 md:h-5 lg:w-4 lg:h-4 text-yellow-400 fill-current"
              viewBox="0 0 20 20"
            >
            ...
            <div
              ...
              style={{
                transform: `translateX(-${
                  currentIndex *
                  (100 /
                    Math.ceil(duplicatedTestimonials.length / cardsPerView))
                }%)`,
                width: `${
                  Math.ceil(duplicatedTestimonials.length / cardsPerView) * 100
                }%`,
              }}
            >
              {Array.from(
                {
                  length: Math.ceil(
                    duplicatedTestimonials.length / cardsPerView
                  ),
                },
                (_, slideIndex) => (
                  <div
                  ...
                  >
                    <div
                      className={`grid gap-6 md:gap-6 lg:gap-6 ${
                        cardsPerView === 1
                          ? "grid-cols-1"
                          : cardsPerView === 2
                          ? "grid-cols-2"
