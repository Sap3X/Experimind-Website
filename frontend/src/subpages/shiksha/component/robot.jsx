              ...
              {features.map((_, index) => {
                ...
                return (
                  <line
                    ...
                    opacity={
                      hoveredFeature === index || animationPhase === index
                        ? "0.8"
                        : "0.3"
                    }
                  />
...
const HexagonFeature = ({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`w-full h-full bg-gradient-to-r ${feature.color} rounded-full`}
        ></div>
      </div>

      {/* Hexagon Shape */}
      <div
        className={`${size} relative transform transition-all duration-500 ${
      ...
      >
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-br ${feature.color} transition-all duration-500`}
          style={{
            clipPath:
              "polygon(25% 6.7%, 75% 6.7%, 100% 50%, 75% 93.3%, 25% 93.3%, 0% 50%)",
          }}
        >
          ...
          <div
            ...
            } bg-blue-700 flex flex-col items-center justify-center ${padding} transition-all duration-300`}
            style={{
              clipPath:
                "polygon(25% 6.7%, 75% 6.7%, 100% 50%, 75% 93.3%, 25% 93.3%, 0% 50%)",
            }}
          >
            {/* Text */}
            <span
              className={`text-white font-bold text-center text-[0.5rem] sm:text-sm  leading-tight transition-all duration-300 ${
...
const RobotCharacter = ({ isMobile = false }) => {
  ...
  return (
    <div className="relative group">
      ...
      {/* Robot container */}
      <div
        className={`relative ${containerSize} flex items-center justify-center transition-all duration-300`}
      >
        <img
        ...
        <div
          className={`absolute ${
            isMobile ? "top-2 left-2 w-3 h-3" : "top-8 left-8 w-6 h-6"
          } bg-gradient-to-r from-yellow-400 to-orange-500 rounded-full animate-bounce opacity-80`}
        ></div>
        <div
          className={`absolute ${
            isMobile ? "top-3 right-3 w-2 h-2" : "top-12 right-12 w-4 h-4"
          } bg-gradient-to-r from-green-400 to-blue-500 rounded-full animate-pulse opacity-70`}
        ></div>
        <div
          className={`absolute ${
            isMobile
              ? "bottom-4 left-4 w-2.5 h-2.5"
              : "bottom-16 left-16 w-5 h-5"
          } bg-gradient-to-r from-purple-400 to-pink-500 rounded-full animate-ping opacity-60`}
        ></div>
        <div
          className={`absolute ${
            isMobile ? "bottom-5 right-2 w-2 h-2" : "bottom-20 right-8 w-3 h-3"
          } bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full animate-bounce opacity-75`}
        ></div>
      </div>

      {/* Speech bubble */}
      <div
        className={`absolute ${bubblePosition} left-1/2 transform -translate-x-1/2 bg-white rounded-2xl px-2 py-1 sm:px-4 sm:py-2 shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300`}
      >
        <p
          className={`text-gray-800 font-semibold ${bubbleText} whitespace-nowrap`}
        >
          Ready to learn with SHIKSHA! 🚀
        </p>
