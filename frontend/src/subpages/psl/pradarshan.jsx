  ...
  return (
    <section className="max-w-6xl mx-auto bg-white border border-yellow-400 rounded-3xl sm:rounded-3xl shadow-xl p-2 sm:p-6 md:p-10 relative overflow-hidden m-9">
      ...
      <div className="text-center mb-8 sm:mb-14">
        <h1 className="Utility-font-Montserrat text-2xl xs:text-3xl sm:text-5xl lg:text-7xl font-extrabold text-[#f6bb33] mb-2 sm:mb-4">
          PRADARSHAN
        </h1>
        <p className="Utility-font-Buenard text-xs xs:text-sm sm:text-lg text-black font-medium">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
        <div className="bg-[#fff8cc] border border-[#ffdd15] rounded-xl p-2 xs:p-4 sm:p-6 shadow-lg flex items-center">
          <div className="rounded-xl h-48 xs:h-64 sm:h-80 md:h-96 w-full bg-gradient-to-br from-yellow-200 to-yellow-300 flex items-center justify-center">
            <img src={im1} alt="" className="object-cover h-full w-full rounded-xl"/>
          </div>
        </div>
        <div className="Utility-font-Monteserrat bg-[#fff8cc] border border-[#ffdd15] rounded-xl p-2 xs:p-4 sm:p-6 shadow-lg">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            {features.map((feature, index) => (
              <motion.li
                ...
                className={`flex flex-col items-center scale-110 text-center p-3 xs:p-4 rounded-xl transition-all duration-300 ${
        ...
        <div className="Utility-font-NunitoSans flex flex-col sm:flex-row justify-center items-center gap-4 w-full">
          {levelData.map((level, i) => (
            <motion.div
              ...
              className={`relative bg-gradient-to-br ${level.bgColor} rounded-xl p-4 shadow-lg border border-white/60 group transition-all duration-300 hover:shadow-xl flex flex-col items-center justify-between w-80 sm:w-64 md:w-72 lg:w-80 h-30 sm:h-32 md:h-36 lg:h-40 mb-4`}
            >
              {/* Level Badge */}
              <div
